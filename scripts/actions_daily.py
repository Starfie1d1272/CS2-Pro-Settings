        ...
        if not dr:
            upsert_issue(
                ...
                "\n".join([
                    ...
                    f"- observed: {__import__('datetime').date.today().isoformat()}",
                    f"- requested core teams: {manifest.get('requested_core_teams')}",
                    f"- source resolved: {manifest.get('source_resolved_core_teams')}",
                    f"- source unresolved: {manifest.get('source_unresolved_core_teams')}",
                    f"- successful core rosters: {manifest.get('successful_core_team_rosters')}",
                    f"- failed core rosters: {manifest.get('failed_core_team_rosters')}",
                    f"- expected core players: {manifest.get('expected_core_players')}",
                    f"- successful core players: {manifest.get('successful_core_players')}",
                    f"- failed core players: {manifest.get('failed_core_players')}",
                    "",
